...
func (rf *Raft) changeRole(newRole Role) {
	...
	rf.role = newRole
	switch newRole {
	case Role_Follower: // 如果是leader 上面已经变成candidate,下次循环进入case candidate
	case Role_Candidate:
		rf.currentTerm++        //term自增
		rf.votedFor = rf.me     // 给自己投票
		rf.resetElectionTimer() // 重置 Election timeout
	case Role_Leader:
...
func (rf *Raft) ticker() {
	// 开一个匿名go协程，select监听三个channel
	go func() {
		for {
			select { //select语句只能用于channel；区别与switch:随机选择一个case，没有case可运行，则阻塞
			case <-rf.stopCh: // 停止协程，直接退出循环并返回
				return
			case <-rf.applyTimer.C: //Timer定时器channel，接收定时通知。应用日志，向channel发送一个空结构体
				rf.notifyApplyCh <- struct{}{}
			case <-rf.notifyApplyCh: //当有新日志记录提交了，要进行应用
				rf.startApplyLogs()
			}
		}
	}()
	...
	for i, _ := range rf.peers {
		...
		go func(cur int) {
			for rf.killed() == false {
				...
				case <-rf.stopCh:
					return
				case <-rf.appendEntriesTimers[cur].C: // 给每个节点都维护一个计数器，都是150ms，到时间就发送rpc
...
func Make(peers []*labrpc.ClientEnd, me int,
	...
	// start ticker goroutine to start elections.  Raft的心脏
	rf.ticker()
