...
func (rf *Raft) startElection() {
	rf.mu.Lock()
	rf.resetElectionTimer()
	if rf.role == Role_Leader { //是leader直接返回
		rf.mu.Unlock()
		return
	}
	...
	grantedCount := 1
	resCount := 1
	grantedChan := make(chan bool, len(rf.peers)-1) // channel用来收集结果，容量为len-1
	...
	for rf.role == Role_Candidate {
		...
		if grantedCount > allCount/2 { // 超过半数
			//竞选成功
			rf.mu.Lock()
			...
			if rf.role == Role_Leader {
				rf.resetAppendEntriesTimersZero() //当选leader立即发送心跳
			}
			rf.persist()
			rf.mu.Unlock()
